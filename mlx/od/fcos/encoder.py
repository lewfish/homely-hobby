...
def get_stride(box_size, pyramid_shape):
    for stride, max_box_size, _, _ in pyramid_shape:
        if box_size <= max_box_size:
            return stride
    return stride

def make_targets(pyramid_shape, num_labels):
    # setup empty reg and label arrays for each level in pyramid
    targets = {}
    for stride, max_box_size, h, w in pyramid_shape:
...
def encode_targets(boxes, labels, pyramid_shape, num_labels):
    # pyramid_shape is list of (stride, max_box_side, h, w) sorted by
    # stride
    ...
    targets = make_targets(pyramid_shape, num_labels)
