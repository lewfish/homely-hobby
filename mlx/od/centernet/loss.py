...
def loss(head_out, encoded_target):
    ...
    reg_loss = l1_loss(flat_out_reg[is_pos, :], flat_targ_reg[is_pos, :])
    ...
    keypoint_loss = binary_cross_entropy_with_logits(
        flat_out_keypoint, flat_targ_keypoint, reduction='sum') / num_pos

    total_loss = keypoint_loss + 0.1 * reg_loss
